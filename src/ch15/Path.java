...
/**
 * A path in a maze is defined by a starting (row, column) position and a
 * direction (NORTH, EAST, SOUTH, or WEST). A path starts out with the same
 * starting and ending position and direction. It can be extended as the maze is
 * traversed, until it reaches an exit, intersection, or dead end.
 * 
 * Intuitively, think of the path as starting at an intersection and then moving
 * forward, potentially turning along the way, until it reaches another
 * intersection.
 */
public class Path {
	 ...
	 * @return true if they are opposites (i.e. 0 and 2, 1 and 3, 2 and 0, or 3 and
	 *         1)
	 */
	...
	public boolean isOpposite(Path other) {
		return startingRow == other.endingRow && startingColumn == other.endingColumn
